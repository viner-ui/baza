...
<div class="page-header">
    ...
    <p>Заявки, назначенные на вас. Взять в работу или завершить.</p>
</div>
                ...
                <tr>
                    ...
                    <td class="actions-cell">
                        @if($r->status === 'assigned')
                            <form action="{{ route('master.take', $r->id) }}" method="POST" class="form-inline">
                                @csrf
                                <button type="submit" class="btn btn-sm">Взять в работу</button>
                            </form>
                        @endif
                        @if($r->status === 'in_progress')
                            <form action="{{ route('master.complete', $r->id) }}" method="POST" class="form-inline">
                                @csrf
